...
public:
 Editor(void* c){ ViewRect r(0,0,1400,900); rect=r; }
 Steinberg::tresult PLUGIN_API attached(void* p, Steinberg::FIDString t) SMTG_OVERRIDE;
 Steinberg::tresult PLUGIN_API removed() SMTG_OVERRIDE;
 Steinberg::tresult PLUGIN_API getSize(Steinberg::ViewRect* s) SMTG_OVERRIDE;
 Steinberg::tresult PLUGIN_API onSize(Steinberg::ViewRect* ns) SMTG_OVERRIDE;
 static void* createInstance(void* c){return new Editor(c);}
private:
 HWND hwnd=nullptr;
};
